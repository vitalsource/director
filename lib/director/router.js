...
// #### @path {String}
// #### @return {Object}
// Parses the query string parameters from a path string and
// returns them as an Object.
//
// e.g.
//
//    new Router().parseQuery('foo/bar?baz=1&qux&baz=2');
//
// yields
//
//    { 'baz': ['1', '2'], 'qux': true }
...
Router.prototype.parseQuery = function(path) {
  ...
  var query = match[0].substr(1),
      data = {},
      decode = decodeURIComponent;

  if (query.length) {
    query.split(/&/g).forEach(function(part) {
      // 'x==y' should yield {x: '=y'}
      var bits = part.split('='),
          key = decode(bits.shift()),
          value = bits.length
            ? decode(bits.join('='))
            : true;
      set(key, value);
